...
function NewServer(props) {
  ...
  const validate = {
    ...
    votifier_token: () => {
      if (!details.votifier_token) {
        return true;
      }
      if (details.votifier_token.length < 15) {
        return "Your server Votifier token must be more than 15 characters in length!";
      }
      if (details.votifier_token.length > 220) {
        return "Your server Votifier token must not be more than 220 characters in length!";
      }
      return true;
    },
  };
  ...
  const submit = async () => {
    ...
    addToast(`Successfully edited your server!`, {
      appearance: "success",
    });
