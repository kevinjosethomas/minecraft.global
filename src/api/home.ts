...
async function GetHomeResults() {
  ...
  try {
    const homepage = await axios.get(process.env.LOCAL_API + "/homepage");
    return [
      {
        sponsored: homepage.data.payload.premium_servers,
        popular: homepage.data.payload.popular_servers,
        newly: homepage.data.payload.new_servers,
      },
      null,
    ];
  } catch (e) {
